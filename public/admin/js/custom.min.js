...
function show_message_for_image_loading() {
    let image = $('[image_id][loaded=0]').first();
    if (image.length) {
        new PNotify({
            type: 'info',
            title: '',
            text: 'Не закривайте цю сторінку поки всі фото не завантажаться. Поки завантажується можна переглядати, редагувати та зберігати',
            styling: 'bootstrap3',
        });
    }
}

function show_message_for_image_loading_end() {
    let image = $('[image_id][loaded=0]').first();
    if (image.length === 0) {
        new PNotify({
            type: 'info',
            title: '',
            text: 'Всі фото завантажені, можна закривати сторінку',
            styling: 'bootstrap3',
        });
    }
}
...
function tryDownload() {
    let image = $('[image_id][loaded=0]').first();
    if (image.length) {
        let image_id = image.attr('image_id');
        let type = $('#image-type').val();
        let url;
        if (type === 'hike') {
            url = site_url + 'image/processimage/' + image_id;
        } else {
            url = site_url + 'image/processdynamicimage/' + image_id;
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.error) {
                    ...
                    image.attr('src', site_url + 'image' + response.horizontal);
                    image.attr('loaded', 1);
                }
                show_message_for_image_loading_end();
                if (!response.stop) {
                    tryDownload();
                }
            }
...
function attachEditor(id, height) {
    var editor = editormd(id, {
        // width  : "100%",
         height : height,
        path: site_url + "admin/vendors/editor.md-master/lib/",
...
function initEditor() {
    $(function() {
        $('.textarea-editor:visible').each( function() {
            let id = $(this).attr('id');
            let height = '400px';
            attachEditor(id, height);
        });
        $('a[data-toggle="pill"]').on('shown.bs.tab', function (e) {
            let el = $(e.target);
            let href = el.attr('href');
            let widget_id = el.attr('id');
            history.pushState('', '', '#' + widget_id);
            
            let editor_container = $(href).find('.textarea-editor');
            editor_container.each( function() {
                let height = '400px';
                attachEditor($(this).attr('id'), height);
            });
...
jQuery(document).ready(function ($) {
    ...
    activatePill();
    initEditor();
    show_message_for_image_loading();
});